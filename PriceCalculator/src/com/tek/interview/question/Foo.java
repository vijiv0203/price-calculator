	...
	public void calculate(Map<String, Order> o) {
		...
		for (Map.Entry<String, Order> entry : o.entrySet()) {
			...
			for (int i = 0; i < r.size(); i++) {
				...
				if (r.get(i).getItem().getDescription().toUpperCase().contains("imported".toUpperCase())) {
					tax = r.get(i).getItem().getPrice() * 0.15; // Extra 15% tax on
					// imported items
				} else {
				...
				System.out.println("1 " + r.get(i).getItem().getDescription() + ": " + (double) Math.ceil(totalprice*100.0)/100.0);

				// Keep a running total
				totalTax += tax;
				total += r.get(i).getItem().getPrice()+tax;
			}

			// Print out the total taxes
			System.out.println("Sales Tax: " + Math.ceil(totalTax*100.0)/100.0);

			total = total - Math.round(totalTax*100.0)/100.0;
